...
from dfs import DFS
from UCS import UCS
from BestFirst import BestFS
from Astar import Astar
from IDS import IDS
from BISearch import BIDS
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                if searchType == "BFS":
                    ...
                    graphucs = UCS(G, graphType)
                    ucspath = graphucs.ucsSearch(start, goal)
                    if (ucspath):
                        print("UCS Path: ", end = '')
                        graphucs.ucsTraversal(ucspath)
                        Goal_list.clear()
                    else:
                        print("Path does not exist")
                elif searchType == 'BESTF':
                    bestFirst = BestFS(G, graphType)
                    BFpath = bestFirst.BestFSSearch(start, goal)
                    if BFpath:
                        print("BFS Path: ", end='')
                        bestFirst.BestFSTraversal(BFpath)
                        Goal_list.clear()
                    else:
                        print("Path does not exist")
                elif searchType == 'A*':
                    astar = Astar(G, graphType, self.HeuristicDict)
                    AstarPath = astar.AstarSearch(start, goal)
                    if AstarPath:
                        print("Astar Path: ", end='')
                        astar.astarTraversal(AstarPath)
                        Goal_list.clear()
                    else:
                        print("Path does not exist")
                elif searchType == 'A*':
                    ...
                    ids = IDS(G, graphType)
                    IDSPath = ids.ids(start, goal, 100)
                    if IDSPath:
                        print("IDS Path: ", end='')
                        ids.idsTraversal(IDSPath)
                        Goal_list.clear()
                    else:
                        print("Path does not exist")
                elif searchType == 'Bidirect':
                    bi = BIDS(G, graphType)
                    bidsPath = bi.bids(start, goal)
                    if bidsPath:
                        print("BIDS Path: ", end='')
                        bi.bidsTraversal(bidsPath)
                        Goal_list.clear()
                    else:
                        print("Path does not exist")
