...
class Astar:
    ...
    def astarTraversal(self, bfsPath):
        for i in range(len(bfsPath)-1):
            print(bfsPath[i], end=" ")
        print()
        print("Cost is: ", bfsPath[-1]-self.HeuristicDict[bfsPath[len(bfsPath)-2]])

    def AstarSearch(self, s, g):
        visited = []
        queue = []
        path = []
        queue.append([self.HeuristicDict[s], s])
        visited.append(s)
        ...
        while queue:
            ...
            for neighbor in neighborsInOrder:
                if neighbor not in visited:
                    ...
                    newCost = current[0]-self.HeuristicDict[current[1]]+self.HeuristicDict[neighbor]+neighbors[neighbor]['weight']
                    tempPath = [newCost, neighbor]
